...
export function SectionHead({ eyebrow, lines, intro, className = "", invert = false }: Props) {
  return (
      ...
      {intro ? (
        <Reveal delay={0.12}>
          {/* t-lead carries its own 46ch measure; the old max-w + t-body-caps pair
              is gone, and the composited text-bone/80 is now a named token. */}
          <p className={`t-lead mt-8 ${invert ? "text-bone-dim" : "text-graphite"}`}>{intro}</p>
        </Reveal>
      ) : null}
    </div>
  );
